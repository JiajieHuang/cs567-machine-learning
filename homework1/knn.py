...
def compute_distances(Xtrain, X):
	...
	#				 YOUR CODE HERE					#
	#####################################################	
	size_train=Xtrain.shape
	num_train=size_train[0]
	size_test=X.shape
	num_test=size_test[0]
	dists=np.zeros((num_test,num_train))
	for i in range(0,num_test):
		for  j in range(0,num_train):
			sqr_err=(X[i,:]-Xtrain[j,:])**2
			dists[i,j]=np.sqrt(sqr_err.sum())	 
	return dists	 
...
def predict_labels(k, ytrain, dists):
	...
	ypred=np.zeros((num_train,1))
	for i in range(0,num_test):
		mapping=dict()
		dist_for_one=np.zeros((1,num_train))
		for j in range(0,num_train):
			dist_for_one[0][j]=dists[i][j]
		dist_for_one.sort()
		threshold=dist_for_one[0,k-1]
		for j in range(0,num_train):
			if dists[i][j]<=threshold:
				if ytrain[j] in mapping:
					mapping[ytrain[j]]=mapping[ytrain[j]]+1
				else:
					mapping[ytrain[j]]=1
		maxi=0
		for cat in mapping:
			if mapping[cat]>maxi or (mapping[cat]==maxi and cat<ypred[i]): 
				ypred[i]=cat
				maxi=mapping[cat]
	return ypred
