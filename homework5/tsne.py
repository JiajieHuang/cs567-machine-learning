...
def compute_Q(Y):
    ...
    dist_2= np.add(np.add(-2 * np.dot(Y, Y.T), sum_Y).T, sum_Y)
    Y_sim=1.0/((1.0+dist_2))
    Y_sim=Y_sim-np.eye(N)
    denominator=Y_sim.sum()
    Q=Y_sim/denominator
    return Q, Y_sim
...
def compute_gradient(P, Q, Y_sim, Y, dY, no_dims):
    ...
    # start your code here
    L=(P-Q)*Y_sim
    dY=4*(np.diagflat(L.sum(0))-L).dot(Y)
    return dY
