...
def viterbi(pi, A, B, O):
  ...
  eta_0=np.ones((S,))
  for i in range(0,S):
    eta[i][0]=B[i,O[0]]*pi[i]*eta_0[i]
  for t in range(1,N):
    for j in range(0,S):
      for k in range(0,S):
        production=A[k,j]*eta[k][t-1]*B[j,O[t]]
        if production>eta[j][t]:
          eta[j][t]=production
          parent[j][t]=k
  maxEtaT=0
  for i in range(0,S):
    if maxEtaT<eta[i][N-1]:
      maxEtaT=eta[i][N-1]
      end=i
  path.append(end)
  cur=end
  for n in range(N-1,0,-1):
    cur=int(parent[cur,n])
    path.append(cur)
  path.reverse
  return path
