...
def objective_function(X, y, w, lamb):
    ...
    N,D=X.shape
    cost_of_each_data=np.zeros((N,))
    for i in range(0,N):
        cost_of_each_data[i]=1-y[i]*(X[i,:].dot(w))
        if cost_of_each_data[i]<0:
            cost_of_each_data[i]=0
    obj_value=lamb/2*((w**2).sum())+cost_of_each_data.sum()/N
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        sum=np.zeros((D,1))
        for i in A_t:
            if (ytrain[i]*(Xtrain[i]).dot(w)<1):
               sum=(sum.T+ytrain[i]*(Xtrain[i,:])).T
        w=(1-n_t*lamb)*w+(n_t/k)*sum
        if lamb*(w**2).sum()>1:
            w=1/(np.sqrt(lamb*(w**2).sum()))*w
...
def pegasos_test(Xtest, ytest, w, t = 0.):
    ...
    N,D=Xtest.shape
    linear_output=Xtest.dot(w)
    y_pred=np.zeros(N,)
    for i in range(0,N):
        if linear_output[i]>t:
            y_pred[i]=1
        else:
            y_pred[i]=-1
    test_acc=(y_pred==ytest).sum()/N
