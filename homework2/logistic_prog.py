...
def softmax(w,x):
    p=numpy.exp(w.dot(x))/(numpy.exp(w.dot(x)).sum())
    p=p.T
    return p
...
def logistic_train_ovr(Xtrain, ytrain, w, b, step_size, max_iterations):
    ...
    for i in range(0, max_iterations):
        ...
        b -=  step_size * (1.0 /N) * sum(pred - ytrain)
        gradient=numpy.zeros((D,))
        for j in range(0,N):
            data=numpy.array(Xtrain[j])
            gradient=gradient+(pred[j]-ytrain[j])*data
        gradient=gradient/N
...
def logistic_test_ovr(Xtest, w_l, b_l):
    ...
    # you need to fill in your solution here
    N,D=Xtest.shape
    test_pred=numpy.zeros((1,N))
    test_pred=sigmoid(w_l.dot(Xtest.T)+b_l)
    return test_pred
...
def logistic_mul_train(Xtrain, ytrain, w, b, step_size, max_iterations):
    ...
    g=numpy.zeros((C,D))
    grad_b=numpy.zeros((C,))
    for iter in range(0,max_iterations):
        for i in range(0,N):
            data=Xtrain[i,:]
            fact=ytrain[i,:]
            for j in range(0,C):
                g[j,:]=(softmax(w,data)-fact)[j]*data
            grad=grad+g
            grad_b=grad_b+(softmax(w,data)-fact)
        grad=grad/N
        grad_b=grad_b/N
        w=w-step_size*grad
...
def logistic_mul_test(Xtest, w_l, b_l):
    ...
    for i in range(0,N):
        test_pred[i,:]=softmax(w_l,Xtest[i,:])
    return test_pred
