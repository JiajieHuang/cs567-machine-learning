...
def nb_train_smoothing(X, Y, vocab_size, num_classes, alpha):
    ...
    # Q8.2 Edit here
    ###################################################
    class_word_prob = alpha*np.ones((num_classes, vocab_size))
    N=len(X)
    count=[0,0]
    class_prob[0]=Y.count(0)/len(Y)
    class_prob[1]=Y.count(1)/len(Y)
    for i in range(N):
        voc=len(X[i])
        for j in range(voc):
            word_freq=X[i][j]
            catgory=Y[i]
            class_word_prob[catgory][word_freq[0]]+=word_freq[1]
            count[catgory]+=word_freq[1]
    class_word_prob[0,:]/=(count[0]+vocab_size*alpha)
    class_word_prob[1,:]/=(count[1]+vocab_size*alpha)
...
def nb_train(X, Y, vocab_size, num_classes):
    ...
    for i in range(N):
        ...
        for j in range(voc):
            ...
            class_word_prob[catgory][word_freq[0]]+=word_freq[1]
            count[catgory]+=word_freq[1]
    class_word_prob[0,:]/=count[0]
    class_word_prob[1,:]/=count[1]
    return class_prob, class_word_prob
...
def nb_predict(X, class_prob, class_word_prob):
    ...
    N=len(X)
    for i in range(N):  
        log_pham_x=log(class_prob[0])
        log_pspam_x=log(class_prob[1])
        voc=len(X[i])
        for j in range(voc):
            word_id=X[i][j][0]
            freq=X[i][j][1]
            if class_word_prob[0][word_id]!=0 or class_word_prob[1][word_id]!=0:
                log_pham_x+=freq*log(class_word_prob[0][word_id])
                log_pspam_x+=freq*log(class_word_prob[1][word_id])
        if log_pham_x>log_pspam_x:
            Ypred.append(0)
        else:
            Ypred.append(1)
    return Ypred
